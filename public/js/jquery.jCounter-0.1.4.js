...
(function($,document,window,undefined) {
	...
	$.fn.jCounter = function(options,callback) {
		...
		var settings = {
			...
			date: null,
			debugLog: false,
			serverDateSource: 'dateandtime.html', //path to dateandtime.html file (i.e. http://my-domain.com/dateandtime.html)
			format: 'dd:hh:mm:ss',
		...
		function initCounter(el) {
			if(customDuration) {
				...
				$.ajax({
	                ...
	                error : function(){
						if(consoleLog) { console.log("(jC) Error: Couldn't find dateandtime.html from serverDateSource: " + thisEl.data('userOptions').serverDateSource + "\n(jC) - Make sure the path is correct! \n(jC) - Now using the client-side time (not recommended).") }
